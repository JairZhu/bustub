...
class Trie {
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    char kc = key[key.length() - 1];
    if ((*cur)->HasChild(kc)) {
      cur = (*cur)->GetChildNode(kc);
      if ((*cur)->IsEndNode()) {
        latch_.WUnlock();
        return false;
      }
      TrieNode *np = new TrieNodeWithValue(std::move(*(cur->get())), value);
      *cur = std::unique_ptr<TrieNode>(np);
      latch_.WUnlock();
      return true;
    }
    TrieNode *np = new TrieNodeWithValue(kc, value);
    (*cur)->InsertChildNode(kc, std::unique_ptr<TrieNode>(np));
    latch_.WUnlock();
    return true;
  }
  ...
  bool Remove(const std::string &key) {
    ...
    latch_.WLock();
    bool res = RemoveKey(key, &root_, 0);
    latch_.WUnlock();
    return res;
  }

  bool RemoveKey(const std::string &key, std::unique_ptr<TrieNode> *parent, unsigned int idx) {
    if (!(*parent)->HasChild(key[idx])) {
      return false;
    }
    std::unique_ptr<TrieNode> *cur = (*parent)->GetChildNode(key[idx]);
    if (idx == key.length() - 1) {
      if (!(*cur)->HasChildren()) {
        (*parent)->RemoveChildNode(key[idx]);
      } else {
        *cur = std::make_unique<TrieNode>(std::move(*(cur->get())));
      }
      return true;
    }
    if (RemoveKey(key, cur, idx + 1)) {
  ...
  T GetValue(const std::string &key, bool *success) {
    ...
    auto dt = dynamic_cast<TrieNodeWithValue<T> *>(cur->get());
    if (dt == nullptr) {
      *success = false;
      latch_.RUnlock();
      return {};
